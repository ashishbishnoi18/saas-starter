defmodule SaasStarter.Accounts do
  ...
  alias SaasStarter.Accounts.{User, UserToken, UserNotifier}
  ...
  See `SaasStarter.Accounts.User.email_changeset/3` for a list of supported options.
  ...
  @doc """
  Logs the user in by magic link. Two cases:

    1. Unconfirmed user — confirms them, logs in, expires all their tokens.
    2. Already confirmed — logs in, expires just the consumed magic-link token.

  The unconfirmed-user-with-password case from the original phx.gen.auth
  scaffold doesn't apply here because we don't support password signup.
  """
